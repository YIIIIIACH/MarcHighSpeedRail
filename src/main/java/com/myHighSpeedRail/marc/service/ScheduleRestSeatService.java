...
public class ScheduleRestSeatService {
	 ...
	 * orderSeatList;
	 */
	public Boolean updateScheduleRestSeat(Integer schid, Integer discountid, Integer rrid, Integer ststid, Integer endstid,Integer ticketCnt) {
		...
		if( tmp1.size()<=0 || tmp2.size()<=0)return false;
		rrs1 = tmp1.get(0);
		rrs2 = tmp2.get(0);
		Integer rrs1seq= rrs1.getRailRouteStopStationSequence();
		Integer rrs2seq =rrs2.getRailRouteStopStationSequence();
		if( rrs2seq>= rrs1seq) return false;
		if( rrs1.getStopStation().getStationId()== rrs2.getStopStation().getStationId())return false;
		...
		if(tarSchrs.get(0).getRestSeatAmount() < ticketCnt) {
			System.out.println("[Fatal]: did not have enought seat");
			return false;
		}
